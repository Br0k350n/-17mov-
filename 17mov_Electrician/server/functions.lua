...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    if Config.Framework == "ESX" then
        ...
        for k,v in pairs(xPlayers) do
            identifiers[#identifiers+1] = v.identifier
        end
        ...
        if #identifiers > 0 then
            ...
            for i = 1, #identifiers do
                placeholders[#placeholders+1] = '@id' .. i
            end
            ...
            MySQL.Async.fetchAll(("SELECT identifier, electrocourses FROM users WHERE identifier IN (%s)"):format(table.concat(placeholders, ',')), params, function(result)
    ...
    elseif Config.Framework == "QBCore" then
        ...
        local xPlayers = Core.Functions.GetQBPlayers()
        local citizenids = {}

        for k,v in pairs(xPlayers) do
            citizenids[#citizenids+1] = v.PlayerData.citizenid
        end
        ...
        if #citizenids > 0 then
            ...
            for i, id in ipairs(citizenids) do
                params['@id' .. i] = id
            end

            MySQL.Async.fetchAll(("SELECT citizenid, electrocourses FROM players WHERE citizenid IN (%s)"):format(table.concat(placeholders, ',')), params, function(result)
                for _, v in pairs(result) do
...
AddEventHandler('QBCore:Server:PlayerLoaded', function(xPlayer)
    MySQL.Async.fetchAll('SELECT electrocourses FROM players WHERE citizenid = @identifier', {
...
AddEventHandler('playerDropped', function()
    ...
    if Config.Framework == "QBCore" then
        ...
        if player ~= nil and player.PlayerData ~= nil then
            ...
            if PlayerCourses[identifier] then
                MySQL.Async.execute('UPDATE players SET electrocourses = @electroCourses WHERE citizenid = @identifier', {
                    ['@electroCourses'] = PlayerCourses[identifier],
                    ['@identifier'] = identifier
                })
                PlayerCourses[identifier] = nil
            end
        end
    end
end)
...
AddEventHandler('esx:playerLoaded', function(source)
    if source ~= nil then
        local xPlayer = Core.GetPlayerFromId(source)
        MySQL.Async.fetchAll('SELECT electrocourses FROM users WHERE identifier = @identifier', {
...
AddEventHandler('esx:playerDropped', function(source)
    if source ~= nil then
        ...
        if courses then
            ...
            PlayerCourses[xPlayer.identifier] = nil
        end
